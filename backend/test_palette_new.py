...
def _get_dominant_colors(image: Image.Image, num_colors: int = 5) -> list[str]:
    ...
    """
    small = image.copy()
    small.thumbnail((150, 150))  # shrink first — full resolution isn't needed for this
...
def extract_combined_palette(images: list[bytes], num_colors: int = 5) -> list[str]:
    ...
    then running the same extraction on that combined canvas.
    """
    thumbnails = []
...
if __name__ == "__main__":
    ...
    print(extract_palette(image_bytes))

    print("\n=== Combined palette (same image x2, simulating a carousel) ===")
    print(extract_combined_palette([image_bytes, image_bytes]))